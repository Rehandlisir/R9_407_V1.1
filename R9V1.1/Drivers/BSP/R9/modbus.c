...
uint16_t Reg[200] ={0x0000
           };//reg是提前定义好的寄存器和寄存器数据，要读取和改写的部分内容
 

void Modbus_UX_IRQHandler(void)
{
    uint8_t res;
 ...
 * @retval      无
 */
void modbusUART2_init(uint32_t baudrate)
{
    /* IO 及 时钟配置 */
    ...
    HAL_NVIC_SetPriority(Modbus_UX_IRQn, 1, 2);              /* 抢占优先级3，子优先级3 */
...
{
    Modbus_RE(1);                                            /* 进入发送模式 */
    HAL_UART_Transmit(&g_modbus_handler, buf, len, 1000);    /* 串口2发送数据 */
//    g_Modbus_rx_cnt = 0;
    Modbus_RE(0); /* 进入接收模式 */
}
/******************************************************************************************/
...
void Modbus_Func3()
{
	...
	for(j=0;j<i;j++)//发送数据
	{
	  modbus_send_data(&modbus.sendbuf[j],1);	
	}
